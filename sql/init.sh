...
# MySQLを初期化
cat /home/isucon/webapp/sql/initdb.d/00_create_database.sql | mysql -u"$ISUCON_DB_USER" \
...
echo 'CREATE INDEX records_idx ON records(`disabled`,`name`,`domain_id`);' | mysql -u"$ISUCON_DB_USER" \
		-p"$ISUCON_DB_PASSWORD" \
		--host "$ISUCON_DB_HOST" \
		--port "$ISUCON_DB_PORT" \
		"isudns"
